...
MV.COD_ITEM PRODUCTID,
 COALESCE(i.PART_NUMBER,i.COD_ITEM) as mpn,
  I.DESCRICAO name,
  MV.COD_FORNECEDOR,
  MV.COD_EMPRESA,
  i.COD_GTIN AS gtin,
  COALESCE (iff.Custo_Contabil ,IC.PRECO_VENDA, ic.custo_contabil) AS sellPrice,
...
INNER JOIN FORNECEDOR_ESTOQUE fe ON fe.COD_FORNECEDOR = mv.COD_FORNECEDOR
...
where i.STATUS = 'A'
AND igi.COD_GRUPO_INTERNO NOT IN (8)
AND mv.COD_EMPRESA IN (61)
AND icc.cod_classe_contabil NOT IN (98, 99)
and mv.COD_FORNECEDOR IN (2);
